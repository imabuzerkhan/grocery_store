  ...
  return (
    <div className='md:flex gap-4 items-center'>
      ...
      {business.image && business.image.length > 0 && (
              <Image
                src={business.image[0]?.url || '/path/to/default/image.jpg'}
      ...
      <div className='flex justify-between items-center w-full'>
        <div className='flex flex-col mt-4 md:mt-0 items-baseline gap-3'>
          <h2 className='text-primary p-1 px-3 text-lg bg-purple-100 rounded-full'>
            {business.category?.name || 'Category not available'}
          </h2>
          <h2 className='text-[40px] font-bold'>{business.name || 'N/A'}</h2>
          <h2 className='flex gap-2 text-lg text-gray-500'>
            <MapPin /> {business.address || 'N/A'}
          </h2>
          <h2 className='flex gap-2 text-lg text-gray-500'>
            <Mail /> {business.gmail || 'N/A'}
          </h2>
        </div>
        <div className='flex flex-col gap-5 items-end'>
          ...
          <h2 className='flex gap-2 text-xl text-primary'>
            <User /> {business.contactPerson || 'Contact person not available'}
          </h2>
